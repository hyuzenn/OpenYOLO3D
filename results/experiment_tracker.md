...
## 2026-07-20 | `2026-07-18_e1_grid_v01` — E1: OV-TCS vs GT MOT metrics (preregistered, 21 variants, 146 scenes)

Preregistered validation (`2026-07-18_e1_prereg_v01/PREREGISTRATION.md`, frozen pre-results)
of OV-TCS = L_norm×(1−CSR) against GT-based HOTA/AssA/DetA/IDF1/AMOTA (nuScenes GT instance
tokens; TrackEval ≤2m class-agnostic + official devkit AMOTA). 21/21 variants clean; stats run
once (seed 20260718, 10k scene bootstrap). Full report: `2026-07-18_e1_grid_v01/REPORT.md`.

**Verdict: CONCERNING (frozen §5 rules) → analysis-paper reframing activated.**
- ρ_s(OVTCS, AssA) = **+.011** vs ρ_s(L_norm, AssA) = **+.751**; confirmatory Steiger FAILS
  reversed (t=−3.21, p=.998). Partial ρ(C|L_norm)→AssA = −.02 (ns).
- (1−CSR) is *negatively* correlated with AssA (−.471): class-aware ego association ⇒ CSR≡0
  by construction while fragmentation is worst (structural confound; mechanism verified).
- OV-TCS's top target is DetA (+.596, Steiger p=.006): detection proxy, not association.
- ego→global intervention sign-reversed vs GT for gamma & hybrid (Concerning trigger).
- Disclosures: global-frame `--association-class-agnostic` is a no-op (2 duplicate cells);
  hybrid cache shares gamma geometry, so `hybrid_global_fuse` ≡ gamma_global geometry
  (label-only variant); AMOTA floor 0.000 for 7 YOLO-scored variants.
- Practical GT-free AssA proxy = plain L_norm (.75); entropy formulation A best for
  HOTA/IDF1 (.775/.771, supplement-only). Secondary frag-inject arms (p=.1/.2/.3/.5, never
  pooled): OV-TCS rank-monotone with AssA (5/5) but entirely via L_norm — 1−CSR pinned at 1.0
  across all arms (stability factor inert under class-aware fragmentation); supports verdict.

## 2026-07-28 — E2 score-threshold-matched control (job 107888, `results/2026-07-28_e2_thrmatch_v01`)
- Reviewer objection ("temporal layer only reduces detection count") tested at **exactly matched
  emitted box count** (rel. err +0.0000% both arms): baseline + `--proposal-score-threshold t`
  vs `phase1` (M11 N=3), same cache/GT/evaluator/association frame, nuScenes val-150.
  t = 0.251859 (ego, 221,297 boxes) / 0.153948 (global, 503,619 boxes).
- **Verdict NO for per-frame quality.** At matched count the threshold control beats the gate:
  ego HOTA .2417 vs .2056 (Δ−.0361), DetA .2811 vs .1498 (Δ−.1313, gate wins 0/150 scenes),
  IDF1 .1878 vs .1708, recall .4728 vs .2818, mAP .3213 vs .1360;
  global HOTA .2729 vs .2456, DetA .1737 vs .1448, IDF1 .2808 vs .2422, mAP .3367 vs .2569.
  All paired Wilcoxon p<1e-10, rank-biserial ≈ −1.0, 10k scene-bootstrap CIs exclude 0.
- **Gate wins only identity-level metrics:** AMOTA +.0226 (ego) / +.0263 (global);
  AssA +.0756 ego (110/150 scenes) but −.0116 global.
- Mechanism: the gate selects on persistence, not evidence quality (suppresses first N−1 frames
  of every true track); GT fragmentation unchanged from baseline (10.52 / 4.45) since the gate
  does not alter the associator partition, while the control lowers it (8.69 / 3.89).
- **Confound to clear (E2b):** arms matched on boxes, not tracks — gate emits 64,397 / 107,920
  tracks vs control 123,992 / 149,054, so the AMOTA/AssA gain may be an identity-count effect.
  Track-count-matched control required before the AMOTA claim is publishable.
- Side note: thresholding lowers OV-TCS_C (.194→.159 ego, .188→.196 global) while the gate
  raises it (.380/.464) — OV-TCS_C follows count reduction, consistent with the E1 CONCERNING verdict.
