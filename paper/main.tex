...
24\% (mean per-scene gain $+0.049$, 95\% CI $[0.045, 0.052]$) and reducing
fragmentation by 58\%, an effect that replicates on a fully open-vocabulary
detection stream. Code and evaluation protocols will be released.
\end{abstract}
...
at all --- yet differ substantially in temporal label quality: OV-TCS $+24\%$
(mean per-scene gain $+0.049$, 95\% CI $[0.045,0.052]$, $96.7\%$ of scenes
...
ego- vs.\ global-frame association, OV-TCS $+24\%$, fragmentation $-58\%$ at
...
computed on ($L\ge2$), any intervention that filters that population can move
...
while \emph{lowering} mAP ($0.341\!\to\!0.310$) --- it simply deletes weak,
...
must be read as filtering, not improvement.
...
with the association-driven metrics --- HOTA $r{=}0.87$ (ego arm; pooled
$0.78$), AssA $0.77$ pooled, IDF1 $0.71$ pooled --- and only weakly with the
...
at bit-identical detection quality. The improvement is not driven by a few
scenes: a scene-level paired bootstrap over the 150 validation scenes gives a
mean per-scene OV-TCS gain of $+0.049$ with $95\%$ CI $[0.045, 0.052]$
($10^4$ resamples), and $96.7\%$ of scenes improve individually. Conversely, the merge and fusion axes
