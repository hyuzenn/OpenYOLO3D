...
95\% CI $[0.142, 0.152]$ over 8{,}046 instances) --- and it replicates on a
fully open-vocabulary detection stream ($+26\%$). Code and evaluation
...
opposite axis --- mAP $0.3408\to0.3420$ at unchanged track topology --- so
...
identities over a fixed taxonomy. Open-vocabulary \emph{tracking} benchmarks
such as TAO~\cite{tao} and OVTrack's evaluation protocol~\cite{ovtrack}
broaden the category set, but still score against annotated GT trajectories
on a fixed benchmark vocabulary --- they extend the taxonomy, not the
GT-free setting. They are all the right tools when GT identities
...
10-class stream; the ``native'' anchor, mAP $0.3408$/NDS $0.3150$) and from a
...
$0\!\to\!0.5$ moves mean OV-TCS $0.476\!\to\!0.438$ (mean over the $L\ge2$
tracks the injection acts on) while mAP does not move at all. As a positive control, corrupting association
...
length --- and the direction of the effect is estimated with high
...
direction it claims. (Sec.~\ref{sec:product} shows the stability factor
alone carries even more of this particular signal --- and why that does not
make it a better metric.)
...
(official devkit). Frag here is CLEAR-MOT's interruption count over
GT-matched trajectories --- a different quantity from the GT-instance
fragmentation of Tab.~\ref{tab:main}, and roughly constant here because both
arms match a similar set of GT trajectories.}
...
\midrule
Per-frame baseline & \textbf{0.3408} & \textbf{0.3150} & ---$^a$ & --- & --- & --- \\
Ego assoc. & 0.3408$^b$ & 0.3150$^b$ & 0.136 & 3.01 & 10.64 & 0.507 \\
Global assoc. & 0.3408$^b$ & 0.3150$^b$ & \textbf{0.188} & \textbf{3.86} & \textbf{4.45} & \textbf{0.457} \\
...
$^e$evaluated under an earlier revision of the detection evaluator whose
anchor read $0.3407$/$0.3145$ (offset $\le 5{\times}10^{-4}$ from the current
anchor); the merge rows' deltas are unaffected.}
\label{tab:main}
\end{table}
...
15-class prompt vocabulary), the same associator change --- the same
production implementations as Tab.~\ref{tab:main} --- reproduces the same
signature: OV-TCS $0.216\!\to\!0.272$ ($+26\%$), fragmentation $-35\%$
($3.76\!\to\!2.43$ predicted tracks per GT instance), switch rate over
multi-frame tracks $-0.041$, at unchanged detection metrics (as in
Tab.~\ref{tab:main}, footnote~$b$: association neither adds nor removes
boxes, so mAP/NDS are quoted from the shared cached evaluation of the frozen
proposal set). Restricting the vocabulary to the 10 nuScenes classes leaves
the deltas essentially unchanged ($+25\%$, $-35\%$), so the improvement is
not carried by non-benchmark ``stuff'' classes. The effect is somewhat
smaller than on the closed stream --- open-vocabulary proposals are noisier
and shorter-lived --- but its direction and structure replicate in the regime
the metric is designed for.
...
$0.3408\!\to\!0.3420$ and NDS $0.3150\!\to\!0.3159$ against the CenterPoint
...
\bibitem{tao} A.~Dave et al. TAO: A large-scale benchmark for tracking any
object. ECCV 2020.
\bibitem{ovtrack} S.~Li et al. OVTrack: Open-vocabulary multiple object
tracking. CVPR 2023.
\end{thebibliography}
}
