"""β1 orchestrator — pillar foreground sweep + HDBSCAN re-measurement.

Single PBS job. NuScenes loaders are paid once; the 50-sample cache is
shared across all 24 combos. HDBSCAN config is locked to W1.5 best to
isolate the foreground-extraction effect.
"""

from __future__ import annotations

import argparse
import itertools
import json
import os
import os.path as osp
import shutil
import sys
import time
import traceback

import numpy as np

from diagnosis_w1.run_clustering_check import _build_loader, _cache_samples
from preprocessing.pillar_foreground import PillarForegroundExtractor
from adapters.lidar_proposals import LiDARProposalGenerator
from diagnosis_beta1.measurements import (
    HDBSCAN_BEST, SampleTimeout, measure_with_timeout, PER_SAMPLE_TIMEOUT_S,
)
from diagnosis_beta1.aggregate import aggregate_beta1, render_all_beta1


# β1 sweep grid (spec)
GRID = {
    "pillar_size_xy": [(0.3, 0.3), (0.5, 0.5), (1.0, 1.0)],
    "z_threshold":    [0.2, 0.3, 0.5, 0.8],
    "ground_estimation": ["ransac", "percentile"],
}


def _combo_id(combo: dict) -> str:
    px, py = combo["pillar_size_xy"]
    return (f"pillar{px:g}x{py:g}_zthr{combo['z_threshold']:g}_"
            f"{combo['ground_estimation']}")


def _verify_hdbscan_regression(cache: dict) -> dict:
    """Run W1.5 best HDBSCAN config on RAW PC (no foreground) and verify
    mean_n_clusters reproduces W1.5's saved 189.7 to 4 decimals.
    """
    gen = LiDARProposalGenerator(**HDBSCAN_BEST)
    ns = []
    for tok in sorted(cache.keys()):
        out = gen.generate(cache[tok]["pc_ego"])
        ns.append(out["n_clusters"])
    mean_n = float(np.mean(ns))
    return {
        "checked_samples": len(ns),
        "mean_n_clusters": mean_n,
        "expected": 189.7000,
        "delta": abs(mean_n - 189.7000),
        "passes": abs(mean_n - 189.7000) < 1e-3,
    }


def _run_combo(combo: dict, cache: dict, hdbscan_gen: LiDARProposalGenerator,
               out_dirs: dict) -> dict:
    cid = _combo_id(combo)
    combo_dir = osp.join(out_dirs["per_sample_per_config"], cid)
    os.makedirs(combo_dir, exist_ok=True)

    extractor = PillarForegroundExtractor(**combo)

    per_sample = []
    timeouts = 0
    errors = 0
    for tok in sorted(cache.keys()):
        try:
            rec = measure_with_timeout(extractor, hdbscan_gen, cache[tok])
            with open(osp.join(combo_dir, f"{tok}.json"), "w") as f:
                json.dump(rec, f, indent=2,
                          default=lambda o: float(o) if hasattr(o, "item") else str(o))
            per_sample.append(rec)
        except SampleTimeout:
            timeouts += 1
        except Exception as e:
            errors += 1
            print(f"  [combo {cid} | {tok}] error: {e}")

    if not per_sample:
        return {"combo": combo, "combo_id": cid,
                "n_samples_succeeded": 0, "n_timeouts": timeouts, "n_errors": errors}

    n_cl = [r["n_clusters"] for r in per_sample]
    M = [r["M_rate"] for r in per_sample]
    L = [r["L_rate"] for r in per_sample]
    D = [r["D_rate"] for r in per_sample]
    miss = [r["miss_rate"] for r in per_sample]
    fg_ratio = [r["foreground"]["ratio"] for r in per_sample]
    timing = [r["timing_total"] for r in per_sample]
    return {
        "combo": combo,
        "combo_id": cid,
        "n_samples_succeeded": len(per_sample),
        "n_timeouts": timeouts,
        "n_errors": errors,
        "mean_n_clusters": float(np.mean(n_cl)),
        "median_n_clusters": float(np.median(n_cl)),
        "mean_M_rate": float(np.mean(M)),
        "mean_L_rate": float(np.mean(L)),
        "mean_D_rate": float(np.mean(D)),
        "mean_miss_rate": float(np.mean(miss)),
        "mean_foreground_ratio": float(np.mean(fg_ratio)),
        "median_timing_total": float(np.median(timing)),
        "p95_timing_total": float(np.percentile(timing, 95)),
    }


def _select_best(combo_summaries):
    """Tiered selection per spec.

    1) M ≥ 0.40 AND n_clusters ∈ [10, 50] AND timing < 2.5s → max M
    2) M ≥ 0.30 AND timing < 3.0s                          → max M
    3) argmax M overall
    """
    valid = [c for c in combo_summaries if c["n_samples_succeeded"] >= 45]
    if not valid:
        return None, "NO_VALID_COMBOS"
    f1 = [c for c in valid if c["mean_M_rate"] >= 0.40
          and 10 <= c["mean_n_clusters"] <= 50
          and c["median_timing_total"] < 2.5]
    if f1:
        best = max(f1, key=lambda c: c["mean_M_rate"])
        return best, "TIER1_PASS"
    f2 = [c for c in valid if c["mean_M_rate"] >= 0.30 and c["median_timing_total"] < 3.0]
    if f2:
        best = max(f2, key=lambda c: c["mean_M_rate"])
        return best, "TIER2_SOFT_PASS"
    best = max(valid, key=lambda c: c["mean_M_rate"])
    return best, "TIER3_FAIL"


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--use-w1-5-samples",
                        default="results/w1_5_diagnostic_sweep/samples_used.json")
    parser.add_argument("--data-config", default="configs/nuscenes_baseline.yaml")
    parser.add_argument("--output", "--output-dir", dest="output_dir",
                        default="results/diagnosis_beta1")
    args = parser.parse_args()

    out_dir = args.output_dir
    out_dirs = {
        "root": out_dir,
        "per_sample_per_config": osp.join(out_dir, "per_sample_per_config"),
        "figures": osp.join(out_dir, "figures"),
    }
    for d in out_dirs.values():
        os.makedirs(d, exist_ok=True)

    # ---- provenance ----
    with open(args.use_w1_5_samples) as f:
        prov = json.load(f)
    mini_tokens = prov["tokens_by_source"]["mini"]
    trainval_tokens = prov["tokens_by_source"]["trainval"]
    shutil.copy(args.use_w1_5_samples, osp.join(out_dir, "samples_used.json"))

    print("=" * 60)
    print(f"β1 — pillar foreground sweep")
    print(f"  {len(mini_tokens)} mini + {len(trainval_tokens)} trainval = "
          f"{len(mini_tokens) + len(trainval_tokens)} samples (from W1.5)")
    n_combos = (len(GRID["pillar_size_xy"]) * len(GRID["z_threshold"])
                * len(GRID["ground_estimation"]))
    print(f"  sweep: {n_combos} combos")
    print("=" * 60)

    # ---- load samples ----
    mini_loader = _build_loader("v1.0-mini", args.data_config, out_dir, "mini_b1")
    mini_cache, _ = _cache_samples(mini_loader, mini_tokens, "mini")
    del mini_loader

    trainval_loader = _build_loader("v1.0-trainval", args.data_config, out_dir, "trainval_b1")
    trainval_cache, _ = _cache_samples(trainval_loader, trainval_tokens, "trainval")
    del trainval_loader

    cache = {**mini_cache, **trainval_cache}
    print(f"  cached {len(cache)} samples")

    # ---- regression check (acceptance #7) ----
    print("\nHDBSCAN regression check (no foreground, W1.5 best config) ...")
    reg = _verify_hdbscan_regression(cache)
    print(f"  mean_n_clusters = {reg['mean_n_clusters']:.4f} "
          f"(expected 189.7000, Δ={reg['delta']:.6f}) "
          f"→ {'PASS' if reg['passes'] else 'FAIL'}")
    with open(osp.join(out_dir, "hdbscan_regression.json"), "w") as f:
        json.dump(reg, f, indent=2)
    if not reg["passes"]:
        print("  ABORT — adapter/HDBSCAN behaviour drifted from W1.5 baseline.")
        sys.exit(2)

    # ---- sweep ----
    hdbscan_gen = LiDARProposalGenerator(**HDBSCAN_BEST)

    combos = []
    for ps, zt, ge in itertools.product(
        GRID["pillar_size_xy"], GRID["z_threshold"], GRID["ground_estimation"]
    ):
        c = {"pillar_size_xy": ps, "z_threshold": zt, "ground_estimation": ge}
        # default ransac/percentile params come from PillarConfig; only override when needed
        combos.append(c)

    print(f"\n=== sweep: {len(combos)} combos × {len(cache)} samples ===")
    t_sweep_start = time.time()
    summaries = []
    for i, combo in enumerate(combos):
        cid = _combo_id(combo)
        t_c = time.time()
        s = _run_combo(combo, cache, hdbscan_gen, out_dirs)
        elapsed = time.time() - t_c
        summaries.append(s)
        if s["n_samples_succeeded"] == 0:
            print(f"  [{i+1}/{len(combos)}] {cid}: FAILED (no samples)")
        else:
            print(f"  [{i+1}/{len(combos)}] {cid}: "
                  f"M={s['mean_M_rate']*100:.1f}%, "
                  f"miss={s['mean_miss_rate']*100:.1f}%, "
                  f"n_cl={s['mean_n_clusters']:.1f}, "
                  f"fg={s['mean_foreground_ratio']*100:.1f}%, "
                  f"t={s['median_timing_total']:.2f}s "
                  f"({s['n_samples_succeeded']}/{len(cache)} ok, {elapsed:.1f}s)")
    print(f"  sweep finished in {time.time() - t_sweep_start:.1f}s")

    # ---- best ----
    best, verdict = _select_best(summaries)
    sweep_record = {
        "n_samples": len(cache),
        "grid": {k: list(v) if not isinstance(v, list) else v for k, v in GRID.items()},
        "n_combos": len(combos),
        "results": summaries,
        "selection_verdict": verdict,
        "best": best,
        "selection_rule": ("tier1: M≥0.40 and 10≤n_cl≤50 and t<2.5; "
                           "tier2: M≥0.30 and t<3.0; "
                           "tier3 (FAIL): argmax(M)"),
    }
    with open(osp.join(out_dir, "parameter_sweep.json"), "w") as f:
        json.dump(sweep_record, f, indent=2)
    if best is None:
        print("  No valid combo (all <45/50 succeeded). Skipping aggregate/report.")
        return 0
    print(f"\n  selection verdict: {verdict}")
    print(f"  best combo: {best['combo_id']} "
          f"(M={best['mean_M_rate']*100:.1f}%, n_cl={best['mean_n_clusters']:.1f}, "
          f"miss={best['mean_miss_rate']*100:.1f}%, t={best['median_timing_total']:.2f}s)")

    # ---- aggregate + figures + report ----
    agg = aggregate_beta1(summaries, best, sweep_record, reg, cache, out_dirs)
    with open(osp.join(out_dir, "aggregate.json"), "w") as f:
        json.dump(agg, f, indent=2)
    render_all_beta1(summaries, best, sweep_record, reg, agg,
                     out_dirs["figures"], osp.join(out_dir, "report.md"))
    print(f"  → {out_dir}/aggregate.json")
    print(f"  → {out_dir}/report.md")
    return 0


if __name__ == "__main__":
    sys.exit(main())
